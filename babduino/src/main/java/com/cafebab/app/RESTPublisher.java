...
public class RESTPublisher implements Runnable {
	...
	private void sendToCafeBab(final Measure measure) {
		try {
			...
			final Form form = new Form().param("sensor", Integer.toString(measure.getSensor()))
			...
			final Response response = webResource.request(MediaType.APPLICATION_FORM_URLENCODED_TYPE)
